...
// middleware to detect if the user is Rachel
middlewareObj.isRachel = function (req, res, next) {
    // is user logged in
    if (req.isAuthenticated()) {
        // find the campground with the requested id
        if (req.user.username == "Rachel") {
                    next();
        } else {
            req.flash("error", "You are not Rachel!");
            res.redirect("back");
                }
            }
	else {
    ...
    if (req.isAuthenticated()) {
        ...
        Campground.findById(req.params.id, function (err, foundCampground) {
            if (err) {
                ...
                if (foundCampground.author.id.equals(req.user._id) || req.user.username =="Rachel")  {
                    next();
                } else {
...
// middleware to check if the user has the food ownership
    ...
    if (req.isAuthenticated()) {
        ...
        Food.findById(req.params.food_id, function (err, foundFood) {
            if (err) {
                ...
                // does the user own the food?
                if (foundFood.author.id.equals(req.user._id)|| req.user.username == "Rachel") {
                    next();
                } else {
                    req.flash("error", "You don't have permission to do that!");
                    res.redirect("back");
    ...
    if (req.isAuthenticated()) {
        ...
        Comment.findById(req.params.comment_id, function (err, foundComment) {
            if (err) {
                ...
                if (foundComment.author.id.equals(req.user._id)|| req.user.username == "Rachel") {
                    next();
                } else {
                    req.flash("error", "You don't have permission to do that!");
                    res.redirect("back");
